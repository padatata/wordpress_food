...
function portfolio_generate_image_links( $item_data, $item, $settings ){
	...
	$resizer = new Portfolio_WP_Image();
...
function portfolio_check_lightboxes_and_links( $item_data, $item, $settings ) {
...
function portfolio_check_hover_effect( $item_data, $item, $settings ){

	$hover_effect_elements = Portfolio_WP_Helper::hover_effects_elements( $settings['effect'] );
	...
	if ( 'none' != $settings['effect'] ) {
		$item_data['item_classes'][] = $settings['effect'];
	}

	return $item_data;
}

function portfolio_check_custom_grid( $item_data, $item, $settings ) {
