...
class Portfolio_WP_Image{
		...
		if ( $image_full ) {
            ...
            switch ( $type ) {
                ...
                case 'custom-grid':
                    ...
                    break;
                default:
                    $return = apply_filters( "portfolio_resize_image_{$type}", $return, $id, $img_size, $sizes );
                    return $return;
                    break;
            }

		}else{
			return new WP_Error( 'portfolio-gallery-error-no-url', esc_html__( 'No image with this ID.', igp_wp ) );
		}
    ...
    public function get_image_info( $args ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error( 'portfolio-gallery-error-no-url', esc_html__( 'No image URL specified for cropping.', igp_wp ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'portfolio-gallery-error-no-file', esc_html__( 'No file could be found for the image URL specified.', igp_wp ) );
        }
        ...
        if ( ! $size ) {
            return new WP_Error( 'portfolio-gallery-error-no-size', esc_html__( 'The dimensions of the original image could not be retrieved for cropping.', igp_wp ) );
        }
    ...
    public function resize_image( $url, $width = null, $height = null, $crop = false, $align = 'c', $quality = 100, $retina = false, $data = array(), $force_overwrite = false ) {
        ...
        // Filter args
        $args = apply_filters( 'igp_gallery_resize_image_args', $args );
        ...
        if ( ! file_exists( $dest_file_name ) || ( file_exists( $dest_file_name ) && $force_overwrite ) ) {
            ...
            if ( is_wp_error( $editor ) ) {
                return new WP_Error( 'portfolio-gallery-error-no-editor', esc_html__( 'No image editor could be selected. Please verify with your webhost that you have either the GD or Imagick image library compiled with your PHP install on your server.', igp_wp ) );
            }
