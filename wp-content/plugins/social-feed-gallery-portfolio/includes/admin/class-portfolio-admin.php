...
class Portfolio_Admin {
	...
	function __construct() {
		...
		// Show general tab
		add_action( 'portfolio_admin_tab_general', array( $this, 'show_general_tab' ) );
	...
	private function sanitize_image( $image ){
		...
		$image_attributes = apply_filters( 'igp_gallery_image_attributes', array(
			'id',
			'alt',
			'title',
			'description',
	...
	public function save_images(){
		...
		if ( ! wp_verify_nonce( $nonce, 'portfolio-ajax-save' ) ) {
		...
		if ( 'igp_wp' != get_post_type( $gallery_id ) ) {
			wp_send_json( array( 'status' => 'failed' ) );
		}
		...
		$old_images = get_post_meta( $gallery_id, 'portfolio-images', true );
		...
		update_post_meta( $gallery_id, 'portfolio-images', $new_images );
	...
	public function save_image(){
		...
		update_post_meta( $gallery_id, 'portfolio-images', $old_images );
		wp_send_json( array( 'status' => 'succes' ) );
	...
	public function admin_custom_css(){
		...
		<style type="text/css">
			li#menu-posts-portfolio-gallery .wp-submenu li:last-child a {color: #52ad3a;}
		</style>
