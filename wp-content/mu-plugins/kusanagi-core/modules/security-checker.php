	...
	public function load_text() {
		$this->messages = array(
			...
			'wp_config_permission' => array(
				...
				1   => __( 'wp-config.php permission is %1s. Recommend permission is 644.', 'wp-kusanagi' ),
				2   => __( 'Administrator of wp-config.php is %1s.%2s.', 'wp-kusanagi' ),
				3   => __( 'Administrator of wp-config.php is %1s.%2s. Recommend administrator is kusanagi.kusanagi.', 'wp-kusanagi' ),
				999 => __( 'wp-config.php not found.', 'wp-kusanagi' ),
			),
...
#			$message .= '<p class="ok">'.sprintf( $this->get_message( 'wp_config_permission', 2 ), $file_info['group'], $file_info['owner'] ).'</p>';
#		} else {
#			$message .= '<p class="alter">'.sprintf( $this->get_message( 'wp_config_permission', 3 ), $file_info['group'], $file_info['owner'] ).'</p>';
#		}
	...
	private function check_uploads_htaccess() {
		...
		if ( file_exists( $filename ) ) {
			...
			if ( 'kusanagi' === $file_info['group'] && 'kusanagi' === $file_info['owner'] ) {
				$message .= '<p class="ok">'.sprintf( $this->get_message( 'uploads_htaccess', 2 ), $upload_base_dir, $file_info['group'], $file_info['owner'] ).'</p>';
			} else {
				$message .= '<p class="alter">'.sprintf( $this->get_message( 'uploads_htaccess', 3 ), $upload_base_dir, $file_info['group'], $file_info['owner'] ).'</p>';
			}
		} else {
			$upload_base_dir = str_replace( ABSPATH , '', $upload_dir['basedir']).'/';
